...
// by v. Parameter root points to root node of tree.
{
	...
	} else if (k > t->root->key) {			// test actually redundant
...
// treat it as though it points to an integer.
{
	...
	printf("(%ld,%d),", (long int)root->key, *(int*)root->value);
...
{ 
	...
	return intmax(heightleft, heightright)+1;
...
// at least one child (see findParent()).
{
	...
	if (root->key == k) {
		return root;
	} else if (root->leftChild && root->leftChild->key== k) {
		return root;
	} else if (root->rightChild && root->rightChild->key== k) {
		return root;
	} else {
		if (root->key < k) {
...
//	p	- points to parent of node to be deleted.
{
	...
	if (p == n){
		n->leftChild->rightChild=p->rightChild;
		...
		//Put code for thing with no child
	}
