...
Node* rotateRight(Node* root){
    ...
    if (root->leftChild) {
        Node *pTmp = root->leftChild;
        // pTmp's right child may be NULL, but that's not an issue
        root->leftChild = pTmp->rightChild;
        pTmp->rightChild = root;
        // Also need to recalculate height of shifted node
        root->height = calcHeight(root);
        // Also height of new parent
        pTmp->height = calcHeight(pTmp);
        // Returns pTmp. root's parent's child need to be set to pTmp in rebalance()
        return pTmp;
    }
    // Need to consider what happens if root has no left child
    else {
        return root;
    }
}//rotateRight()
...
//---Your code goes here
    ...
    if (root->rightChild) {
        Node *pTmp = root->rightChild;
        // pTmp's right child may be NULL, but that's not an issue
        root->rightChild = pTmp->leftChild;
        pTmp->leftChild = root;
        // Also need to recalculate height of shifted node
        root->height = calcHeight(root);
        // Also height of new parent
...
// Get balance factor - difference between left height and right height
    ...
    hr = (root->rightChild == NULL) ? 0 : root->rightChild->height + 1;
    hl = (root->leftChild == NULL) ? 0 : root->leftChild->height + 1;
	return hl - hr;
}//getBalanceFactor()
...
//---Your code goes here
    hrl = (root->rightChild == NULL) ? -1 : root->rightChild->height;
    hll = (root->leftChild == NULL) ? -1 : root->leftChild->height;
...
// If balancing required, perform necessary rotations.
    ...
    // If bf > 1, left subtree is too big
    if (bf > 1) {
        ...
        if (getBalanceFactor(root->leftChild) > getBalanceFactor(root->rightChild)) {
            ...
            // Rotate child left then parent right
            root->leftChild = rotateLeft(root->leftChild);
            root = rotateRight(root);
        }
        // Check if root is still unbalanced
        
    }
    // If bf < -1, right subtree is too big
    else if (bf < -1) {
        printf("Node %d is unbalanced. ", root->key);
        if (getBalanceFactor(root->leftChild) > getBalanceFactor(root->rightChild)) {
            ...
            // Rotate child right then parent left
